...
-(id)initWithFrameRect:(CGRect)rect ImageArray:(NSArray *)imgArr TitleArray:(NSArray *)titArr
{
	...
	if ((self=[super initWithFrame:rect])) {
        ...
        scrollView.pagingEnabled = YES; //分页模式
...
- (void)autoScroll
{
    [NSTimer scheduledTimerWithTimeInterval:2 target:self selector:@selector(autoScrollAction) userInfo:nil repeats:YES];
...
- (void)autoScrollAction
{
    ...
    if (with >= (scrollView.contentSize.width - self.frame.size.width)) {
        with = 0;
    }
